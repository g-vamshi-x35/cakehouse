...
export default async function DeliveryDashboardLayout({ children }: { children: ReactNode }) {
  const { profile } = await requireRole(["delivery", "owner"]);
      ...
      <aside className="hidden lg:flex w-64 shrink-0 bg-brown-dark text-cream-light flex-col">
        <div className="flex items-center gap-2.5 px-6 py-6">
          ...
          <div>
            ...
            <p className="text-[11px] text-cream-light/50">Delivery Dashboard</p>
          </div>
        </div>
        ...
        <div className="p-3 border-t border-cream-light/10">
          <p className="px-3 text-xs text-cream-light/50 mb-2">{profile?.full_name || "Delivery"}</p>
